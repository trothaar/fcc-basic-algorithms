/* Teresa Rothaar
   Free Code Camp Challenge #267 - Wherefore Art Thou
...
function whatIsInAName(collection, source) {
  // Use filter to compare larger object collection
  // with smaller object source
  var arr = collection.filter(function(item){
    // Loop through each item -if no match, false; otherwise, true
    for(var i in source){
      if(source[i] != item[i]){
        return false;
      }
