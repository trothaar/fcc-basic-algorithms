/* Teresa Rothaar
   Free Code Camp Challenge #286 - Record Collection

   You are given a JSON object representing a part of your
   musical album collection. Each album has several
   properties and a unique id number as its key.
   Not all albums have complete information.

   Write a function which takes an album's id (like 2548),
   a property prop (like "artist" or "tracks"), and a
...
function updateRecords(id, prop, value) {
  ...
  // If value is blank, delete entire property
  ...
  if(value === ""){
   ...
   // If prop isn't tracks and value isn't empty
  // (already checked above), update or set value
  // for that prop and return collection
   if(prop !== "tracks") {
    collection[id][prop] = value;
    return collection;
   }

  // If prop is tracks, value not empty (already
  // checked above), and collection has tracks
  // property, push value onto the end of
  // the tracks array and return collection
  if(prop === "tracks" && collection[id].hasOwnProperty(prop)){
    collection[id][prop].push(value);
    return collection;
  }

  // If property tracks doesn't exist, create it
  // and return collection
